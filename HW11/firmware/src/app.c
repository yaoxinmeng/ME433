...
short z_accel;              // IMU data
short z_IIR, z_MAF, z_FIR;  // Processed data
    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            if(i >= 0){                                         
                ...
                z_accel = (raw_data[13] << 8) | raw_data[12];   // combine 2 bytes to form short
                
                // update MAF and FIR buffers
                update_MAF(z_accel);
                update_FIR(z_accel);
                // get IIR, FIR and MAF values
                z_IIR = get_IIR(z_accel);
                z_MAF = get_MAF();
                z_FIR = get_FIR();
                // update IIR buffer
                update_IIR(z_accel);
