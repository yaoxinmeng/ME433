...
void spi_init() {
    ...
    // Master - SPI1, pins are: SDI1(RA1), SDO1(RB8), SCK1(RB14), SS1(RB15)  
    // since the pic is just starting, we know that spi is off. We rely on defaults here
 
    // setup SPI1
    SPI1CON = 0;                // turn off the spi module and reset it
    SPI1BUF;                    // clear the rx buffer by reading from it
    SPI1BRG = 2;                // baud rate to 8 MHz [SPI1BRG = (48000000/(2*desired))-1]
...
// send a word via spi and return the response
