...
void i2c_init(void){
    ...
    I2C1CONbits.DISSLW = 1; // required due to errata
    ...
    unsigned char name = i2c_master_recv();           // save the value returned
    i2c_master_ack(1);                  // send ack bit
    i2c_master_stop();                  // send stop bit
    TRISBbits.TRISB13 = 0;              // set LED (B13) as output
    LED = 1;
    if(name != WHO_AM_I){               // infinite loop if name does not match
        LED = 0;
        while(1){;}
    }  
}
