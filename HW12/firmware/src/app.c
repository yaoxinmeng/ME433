...
short x_accel, y_accel;     // IMU data
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            x_accel = (raw_data[9] << 8) | raw_data[8];     // combine 2 bytes to form a short
            y_accel = (raw_data[11] << 8) | raw_data[10];   
            x_accel = x_accel + 4000;                       // calibration
            y_accel = y_accel + 3000;
            if (x_accel >= -200 && x_accel <= 200){         // have a resting buffer
                x_accel = 0;
            }
            if (y_accel >= -200 && y_accel <= 200){
                y_accel = 0;
            }
            x_accel = x_accel >> 13;                        // reduce the resolution to 3 bits
            y_accel = y_accel >> 13;
            ...
            if (movement_length > 10) {
                ...
                appData.xCoordinate = (int8_t) x_accel ;
                appData.yCoordinate = (int8_t) y_accel;
                movement_length = 0;
            }
